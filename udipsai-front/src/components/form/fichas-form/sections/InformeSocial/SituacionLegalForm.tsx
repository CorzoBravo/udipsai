...
interface SituacionLegalFormProps {
  data: string;
  onChange: (value: string) => void;
  onValidate?: (isValid: boolean) => void;
}

const SituacionLegalForm: React.FC<SituacionLegalFormProps> = ({
  data,
  onChange,
  onValidate,
}) => {
    ...
    <div className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
          Descripción de la Situación Legal
        </label>
        <textarea
          value={data || ""}
          onChange={(e) => handleChange(e.target.value)}
          placeholder="Describa cualquier situación legal relacionada..."
...
export default SituacionLegalForm;
