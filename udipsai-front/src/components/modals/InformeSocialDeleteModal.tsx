...
import { Trash2, Calendar, User, FileText, AlertTriangle } from "lucide-react";
import { DeleteModal } from "../ui/modal/DeleteModal";

interface InformeSocialDeleteModalProps {
  isOpen: boolean;
  onClose: () => void;
  pacienteId: number;
...
export const InformeSocialDeleteModal: React.FC<InformeSocialDeleteModalProps> = ({
  isOpen,
  onClose,
  pacienteId,
  pacienteNombre,
  ...
  const cargarInformes = async () => {
    try {
      ...
      // Solo mostrar los activos (que no estén eliminados lógicamente)
      const activos = (res || []).filter((inf: any) => inf.activo === true);
      setListaInformes(activos);
    } catch (error) {
      console.error(error);
  ...
  const handleEliminarClick = (id: number, numeroInforme: number) => {
    setIdParaEliminar(id);
    setNumParaEliminar(numeroInforme);
  ...
  const ejecutarEliminar = async () => {
    ...
    try {
      ...
      await fichasService.eliminarInformeSocial(idParaEliminar);
      toast.success(`Informe N° ${numParaEliminar} eliminado correctamente`);
      
      // Actualizar listado local
      const updatedList = listaInformes.filter((inf) => inf.id !== idParaEliminar);
      setListaInformes(updatedList);
      
      // Notificar al componente padre para recargar la tabla principal
      onDeleted();

      // Si no quedan más informes, cerrar el modal
  ...
  return (
    ...
    >
      <div className="bg-red-600 dark:bg-red-950 p-6 text-white flex items-center gap-4">
        ...
        <div>
          <h3 className="text-xl font-bold">Eliminar Informe Social</h3>
      ...
      <div className="p-6 bg-gray-50 dark:bg-gray-900 max-h-[60vh] overflow-y-auto">
        {loading ? (
                ...
                return (
                  ...
                  >
                    ...
                    <Button
                      ...
                      onClick={() => handleEliminarClick(inf.id, listaInformes.length - index)}
          ...
          <div className="py-12 text-center text-gray-500">
            <p>No se encontraron informes activos para este paciente.</p>
          </div>
        )}
      </div>
      ...
      <DeleteModal
        ...
        description={`¿Está seguro que desea eliminar la ficha de Informe Social? Esta acción no se puede deshacer.`}
      />
    </Modal>
  );
};
